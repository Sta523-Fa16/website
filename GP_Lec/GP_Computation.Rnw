...
cols = c( "#8dd3c7","#80b1d3", #blue 
          "#fb8072","#fdb462", #red/orange
          "#bc80bd","#fccde5")  #purple
@
...
legend_txt = c("openblas (4 core)", "openblas (24 core)", "magma (GTX 460)", "magma (K20X)")

x = c(1088, 2112, 3136, 4160, 5184, 6208, 7232, 8256, 9280,10304)
y = data.frame(
      openblas4    = c(0.01,0.06,0.19,0.42,0.83,1.43,2.28,3.39,4.88,6.69),
      openblas24   = c(0.01,0.06,0.18,0.24,0.44,0.75,1.15,1.66,2.30,3.11),
      magma_gtx460 = c(0.01,0.06,0.17,0.37,0.69,1.16,1.80,2.65,3.72,5.05),
      magma_k20x   = c(0.01,0.02,0.03,0.04,0.08,0.11,0.16,0.22,0.30,0.39)
    )

par(mar=c(5, 4, 2, 2) + 0.1)

plot(0,0,type='n',xlab='Matrix size',ylab='Time (sec)',xlim=c(min(x),max(x)),ylim=c(0,max(y)))

for(i in 1:ncol(y))
{
    lines(x,y[,i],col=cols[i],lwd=2, lty=i)  
}

legend("topleft",legend=legend_txt, col=cols[1:4], lwd=2, lty=1:4, cex=0.7, bty="n")
@
\end{center}
...
<<echo=FALSE>>=
plot_lr = function(res, xlim)
{
  par(mar=c(5, 4, 2, 2) + 0.1)

  plot(0,0,type='n',
       xlab='Time (sec)',
       ylab=expression(Error ~~ ('||'%.%'||'[F]) ),
       xlim=xlim,ylim=c(0,max(res$error)))

  m = sort(setdiff(unique(res$method), c("cpu", "gpu", "lr2", "lr2 mod")))

  for(i in 1:length(m))
  {
    x = subset(res, res$method == m[i])
    lines(x$time, x$error, col=cols[i], lwd=2, lty=i)
  }

  abline(v=res[res$method == "cpu",]$time,col="grey",lwd=2)
  abline(v=res[res$method == "gpu",]$time,col="grey",lwd=2)

  legend("topright",
         legend=c("Rand LR", "Rand LR Mod", 
                  "Rand LR (2)", "Rand LR Mod (2)", 
                  "PP", "PP Mod"), 
         col=cols[1:length(m)], 
         lty=1:length(m), 
         lwd=2, cex=0.7, bty="n")
...
\only<1>{
\begin{center}
<<echo=FALSE,out.width="0.95\\textwidth",fig.width=6,fig.height=4.25>>=
load("data/res3.Rdata")

plot_lr(res,c(0,max(res$time)))
@
\end{center}
}
